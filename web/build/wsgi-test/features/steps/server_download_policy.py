...
def step_impl(context):
    ...
    context.policy_id = 'policy_object_11'
    context.policy_md5_id = 'policy_md5_11'
...
def step_impl(context):
    ...
    policy_key, policy_md5_key = wsgi.policy.get_policy_keys(context.cfg,
                                                             context.identifier)
...
def step_impl(context):
    ...
    policy = wsgi.policy.get_policy(context.cfg, context.policy_id)
    assert policy == context.policy

@behave.then('the server uses the index to find the md5 for the policy')
def step_impl(context):
    md5 = wsgi.policy.get_policy_md5(context.cfg, context.policy_md5_id)
    print('md5 is ', md5)
