...
def check_register(config, username):
    ...
    cur.execute('''select dpm_page.url from dpm_page, roles, 
    ...
    if (not comAdmin and not dpmAdmin):
        cur.execute('''select dpm_page.url from dpm_page, user_community, 
                user, status where user.name = ? and user_community.dpm_id =
                dpm_page.dpm_id and status.status = 'approved' and 
